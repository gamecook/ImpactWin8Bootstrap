    ...
    .defines(function ()
    {
        ...
        ig.Game.inject({
            ...
            configureCamera: function () {
                this.camera.trap.size.x = Math.round(ig.system.width / 3);
                this.camera.trap.size.y = Math.round(ig.system.height / 3);
                //var cameraMinY = 0;//this.showStats ? -16 : 0;

                var mapWidth = (this.collisionMap.width * this.collisionMap.tilesize);

                if (ig.system.width >= mapWidth) {
                    // Here we want to center the map since it's smaller then the screen
                    this.camera.min.x = this.collisionMap.tilesize * .5;
                    this.camera.max.x = (mapWidth - ig.system.width) * .5;
                } else {
                    // Here we want to not show more then the edge of the map since it's bigger then the screen
                    this.camera.min.x = 0;
                    this.camera.max.x = mapWidth - ig.system.width;
                }

                var mapHeight = (this.collisionMap.height * this.collisionMap.tilesize);

                if (ig.system.height >= mapHeight) {
                    // Here we want to center the map since it's smaller then the screen
                    this.camera.min.y = this.collisionMap.tilesize * .5;
                    this.camera.max.y = (mapHeight - ig.system.height) * .5;
                } else {
                    // Here we want to not show more then the edge of the map since it's bigger then the screen
                    this.camera.min.y = 0;
                    this.camera.max.y = mapHeight - ig.system.height;
                }
